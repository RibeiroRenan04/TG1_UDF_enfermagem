-- EstagioCheck – schema base. Equivale à migration do EF Core "20260507011153_InitialPostgres"
-- (backend/Migrations/), que a API aplica sozinha no startup: use este arquivo só para criar o
-- schema sem subir a API. Registra a migration em "__EFMigrationsHistory" para o EF não tentar
-- aplicá-la de novo. Idempotente pela migration, não pelas tabelas: depois do 003 as tabelas em
-- inglês não existem mais, e um CREATE TABLE IF NOT EXISTS as recriaria vazias.
...
BEGIN
    IF EXISTS (SELECT 1 FROM "__EFMigrationsHistory"
               WHERE "MigrationId" = '20260507011153_InitialPostgres') THEN
        RAISE NOTICE 'Schema base (InitialPostgres) já aplicado neste banco — nada a fazer.';
        RETURN;
    END IF;
